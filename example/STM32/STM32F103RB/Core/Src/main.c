...
static void app_draw_rgb_bars(void)
{
  UWORD red_end = (UWORD)(HUB75_PANEL_WIDTH / 3U);
  UWORD green_end = (UWORD)((HUB75_PANEL_WIDTH * 2U) / 3U);

  for (UWORD y = 0U; y < HUB75_PANEL_HEIGHT; y++)
  {
    for (UWORD x = 0U; x < HUB75_PANEL_WIDTH; x++)
    {
      UWORD color = BLUE;
...
int main(void)
{
  ...
  HUB75_SetRefreshRate(4); // the parameter is 1-4，the higher the number，the lower the refresh rate is
  App_DrawGuiScreen();
  ...
  while (true)
  {
    ...
    page = (UWORD)((page + 1U) % APP_PAGE_COUNT);
    App_DrawPage(page);
    /* USER CODE END WHILE */
