...
public:
  ...
  */
  ...
  /*!
    @brief  Constructor for panels with custom scan row count, such as
            80x40 (1/20 scan) or 96x48 (1/24 scan).
    @param  a        Address/row-select A pin number.
    @param  b        Address/row-select B pin number.
    @param  c        Address/row-select C pin number.
    @param  d        Address/row-select D pin number.
    ...
    @param  lat      RGB latch pin number.
    @param  oe       Output enable pin number.
    @param  dbuf     If true, display is double-buffered.
    @param  width    Panel width in pixels.
    @param  rows     Number of multiplexed scan rows. Visible height is 2X this value.
    @note   pinlist  (SAMD only) uint8_t array of 6 pin numbers corresponding
                     to upper R, G, B and lower R, G, B pins.
  */
  ...
  */
   void DrawString_CN( uint8_t Xstart, uint8_t Ystart, const char * pString, cFONT* font, uint16_t color);
    /**
   * @fn DrawString_CN
   * @brief Chinese character display
   * @param Xstart  x-axis
   * @param Ystart  y-axis
   * @param pString Display contents
   * @param font font size (only 16,32 and 64 three options)
   * @param color  Chinese character color
   */   
...
private:
  ...
  volatile PortType *clkport;   ///< RGB clock PORT register
  volatile PortType *rgbport[6];///< RGB data PORT registers
  PortType rgbmask[6];          ///< RGB data pin bitmasks
  volatile PortType *rgbport_group[6]; ///< Unique RGB PORT registers
  PortType rgbport_group_mask[6];      ///< Combined mask for each RGB PORT
  PortType rgbport_expand[256][6];     ///< Lookup table for grouped RGB writes
  uint8_t rgbport_group_count;         ///< Number of unique RGB PORT groups
#endif
