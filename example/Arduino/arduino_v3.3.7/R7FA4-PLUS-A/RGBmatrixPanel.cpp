...
#if defined(ARDUINO_ARCH_RENESAS) && RGBMATRIX_RENESAS_ISR_REFRESH
static FspTimer renesas_refresh_timer;
static bool renesas_refresh_started = false;
static void renesas_refresh_isr(timer_callback_args_t *args) {
  (void)args;
  if (activePanel) {
...
// Constructor for panels using custom scan row counts such as
// 80x40 (1/20 scan) and 96x48 (1/24 scan).
RGBmatrixPanel::RGBmatrixPanel(uint8_t a, uint8_t b, uint8_t c, uint8_t d,
  ...
  init(rows, a, b, c, clk, lat, oe, dbuf, width
#if defined(ARDUINO_ARCH_SAMD) || defined(ARDUINO_ARCH_ESP32) || defined(ARDUINO_ARCH_RENESAS)
       ,
       pinlist
#endif
...
void RGBmatrixPanel::begin(void) {
  ...
  *addrcport &= ~addrcmask; // Low
  ...
  // UNO R4: cache direct PORT register pointers/masks for faster refresh.
  uint8_t rgbport_group_index[6];
  rgbport_group_count = 0;
  for (uint8_t i = 0; i < 6; i++) {
    pinMode(rgbpins[i], OUTPUT);
    rgbmask[i] = digitalPinToBitMask(rgbpins[i]);
    rgbport[i] = portOutputRegister(digitalPinToPort(rgbpins[i]));
    *rgbport[i] &= ~rgbmask[i];
    rgbport_group_index[i] = rgbport_group_count;
    for (uint8_t j = 0; j < rgbport_group_count; j++) {
      if (rgbport_group[j] == rgbport[i]) {
        rgbport_group_index[i] = j;
        break;
      }
    }
    if (rgbport_group_index[i] == rgbport_group_count) {
      rgbport_group[rgbport_group_count] = rgbport[i];
      rgbport_group_mask[rgbport_group_count] = 0;
      rgbport_group_count++;
    }
    rgbport_group_mask[rgbport_group_index[i]] |= rgbmask[i];
  }
  for (uint16_t i = 0; i < 256; i++) {
    for (uint8_t j = 0; j < rgbport_group_count; j++) {
      rgbport_expand[i][j] = 0;
    }
    if (i & 0x04)
      rgbport_expand[i][rgbport_group_index[0]] |= rgbmask[0];
    if (i & 0x08)
      rgbport_expand[i][rgbport_group_index[1]] |= rgbmask[1];
    if (i & 0x10)
      rgbport_expand[i][rgbport_group_index[2]] |= rgbmask[2];
    if (i & 0x20)
      rgbport_expand[i][rgbport_group_index[3]] |= rgbmask[3];
    if (i & 0x40)
      rgbport_expand[i][rgbport_group_index[4]] |= rgbmask[4];
    if (i & 0x80)
      rgbport_expand[i][rgbport_group_index[5]] |= rgbmask[5];
  }
#endif
...
#if defined(ARDUINO_ARCH_RENESAS) && RGBMATRIX_RENESAS_ISR_REFRESH
  if (!renesas_refresh_started) {
    uint8_t timer_type = GPT_TIMER;
    int8_t channel = FspTimer::get_available_timer(timer_type);
    if (channel >= 0) {
      bool ok = true;
      ok &= renesas_refresh_timer.begin(TIMER_MODE_PERIODIC, timer_type, channel,
                                        (float)RGBMATRIX_RENESAS_ISR_HZ, 50.0f, renesas_refresh_isr,
                                        nullptr);
      ok &= renesas_refresh_timer.setup_overflow_irq(12);
      ok &= renesas_refresh_timer.open();
      ok &= renesas_refresh_timer.start();
      renesas_refresh_started = ok;
    }
  }
#endif
}
...
void RGBmatrixPanel::DrawString_CN( uint8_t Xstart, uint8_t Ystart, const char * pString, cFONT* font, uint16_t color)
{
...
#define LOOPTIME 350
#define ADDR_SETTLE_US 1
#else
#define ADDR_SETTLE_US 2
#endif
#ifndef CALLOVERHEAD
#define CALLOVERHEAD 60
#endif
...
#endif
#if defined(ARDUINO_ARCH_RENESAS) && RGBMATRIX_RENESAS_ISR_REFRESH
  static volatile bool refresh_busy = false;
  if (refresh_busy) {
    return;
  }
  refresh_busy = true;
  ...
  *latport &= ~latmask; // Latch down
  ...
  if (plane > 0) { // 188 ticks from TCNT1=0 (above) to end of function
    ...
    for (i = 0; i < WIDTH; i++) {
      uint8_t v = *ptr++;
      for (uint8_t j = 0; j < rgbport_group_count; j++) {
        PortType rgb_value = *rgbport_group[j];
        rgb_value &= ~rgbport_group_mask[j];
        rgb_value |= rgbport_expand[v][j];
        *rgbport_group[j] = rgb_value;
      }
      *clkport |= clkmask;
      *clkport &= ~clkmask;
    }
