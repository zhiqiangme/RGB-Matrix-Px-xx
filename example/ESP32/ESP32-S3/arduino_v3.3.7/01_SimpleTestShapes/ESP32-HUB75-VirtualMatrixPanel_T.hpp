...
enum PANEL_SCAN_TYPE {
	...
	TWENTY_SCAN_64PX_HIGH,			///< Twenty-scan mode, 64-pixel high panels (1/20 scan).
	TWENTY_SCAN_32PX_HIGH,			///< Twenty-scan mode, 32-pixel high panels (1/20 scan).
};
...
struct ScanTypeMapping {
	static constexpr VirtualCoords apply(VirtualCoords coords, int panel_pixel_base) 
	{
		...
		// TWENTY_SCAN_64PX_HIGH || TWENTY_SCAN_32PX_HIGH (1/20 scan)
		else if constexpr (ScanType == TWENTY_SCAN_64PX_HIGH || ScanType == TWENTY_SCAN_32PX_HIGH) 
		{
			// 1/20 scan mapping logic
			// For 64px high panels: 64 rows / 20 = 3.2, so we need special handling
			// For 32px high panels: 32 rows / 20 = 1.6, so we need special handling
			
			// Calculate which group of 20 rows we're in
			int row_group = coords.y / 20;
			int row_in_group = coords.y % 20;
			
			// Determine if we're in an even or odd group
			if (row_group % 2 == 0) {
				// Even group: add offset based on pixel base
				coords.x += ((coords.x / panel_pixel_base) * panel_pixel_base);
			} else {
				// Odd group: add offset + panel_pixel_base
				coords.x += (((coords.x / panel_pixel_base) + 1) * panel_pixel_base);
			}
			
			// Remap Y coordinate: move to the correct position within the 20-row group
			coords.y = row_in_group + (row_group / 2) * 20;
		}
